<script lang="ts">
    import { makePolyLinePoints, makeScaledPoints, type RenderGeometry } from '$lib/helpers/svg/polyline.js';
    import type { VerticalTick } from '$lib/helpers/svg/ticks.js';
    import type { WindowedQuotation } from '$lib/types/WindowedQuotation.js';
    ...
    $: {
        askScaledPoints = makeScaledPoints({
            data,
            getMeasurement: (item) => item.averageAskPrice,
            getPrice: (item) => item.averageAskPrice,
    ...
    $: {
        bidScaledPoints = makeScaledPoints({
            data,
            getMeasurement: (item) => item.averageBidPrice,
            getPrice: (item) => item.averageBidPrice,
            getTime: (item) => item.windowEndTime,
            geometry: svgGeometry
        });
...
<div>
    <svg
        ...
        <g id="left-ruler-ticks">
            {#each verticalTicks as {y, label}} y
