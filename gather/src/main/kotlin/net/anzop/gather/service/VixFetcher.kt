...
import net.anzop.gather.dto.economics.VixDto
...
class VixFetcher(
    ...
    private fun handleResponse(response: VixResponse) =
        response
            .observations
            .fold<VixDto, Vix?>(null) { prevVix, dto ->
                val vix = dto.toModel()
                val toSave = vix ?: prevVix?.copy(
                    date = dto.date,
                    carryForwardFrom = prevVix.date
                )

            when (toSave) {
                null -> logger.warn("Skipping Vix observation with invalid value: ${dto.value} for date ${dto.date}")
                else -> marketDataFacade.save(toSave)
            }
